...
Dieser Fehler ist in direkter Abhängigkeit zum Eingangssignal und ist daher periodisch und manifestiert sich als harmonische Verzerrung \autocites{dither}{noise-shaping}.
...
Bei Abtastraten von nur dem doppelten (standardmäßig $44.1\si{kHz}$ oder $48\si{kHz}$) überdecken sich das Spektrum des Rauschens mit dem des Signals immer.
...
% \footnote{Die Länge $N$ kann frei gewählt werden, sie sollte jedoch Teil von $2^x \forall x \in \mathbb{N}_0^+$ sein.}
...
Diese kann numerisch generiert werden \autocite{iso226matlab}.
...
s(m, f) = m \tanh \left( \frac{A(f)}{m} \right)
\end{equation}
...
p(g, m) = \left( \frac{1}{\pi} \int_{0}^{\pi} S(m, f) + g  \partial f \right)^2 = 0
\label{eq:optimize}
\end{equation}
...
Die \gls{ath} Kurve von \citeauthor{iso226} und \citeauthor{iso226matlab} wurde nur als Beispiel verwendet;
