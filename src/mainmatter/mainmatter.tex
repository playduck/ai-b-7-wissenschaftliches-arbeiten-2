...
Eine Quantisierungsfunktion $Q$ wird nach \citeauthor{dither} definiert:
\begin{equation}
...
\end{equation}
Diese gilt für einen arbiträren Eingangswert $z$.
...
Eine ideale Funktion kann aus einer modifizierten Abwandlung der $0$ \gls{phon} \gls{ath} Kurve aus \citefield{iso226}{number} gewonnen werden.
...
Es kann nun durch \gls{hard-clipping} oder \gls{soft-clipping} ein Maximum der Funktion festgelegt werden.
Beispielsweise können wir die folgende Funktion zum \gls{soft-clipping} mit dem Maximum $m$ verwenden:
\begin{equation}
\setequationentry{\gls{soft-clipping} Sättigungsfunktion}
...
Dafür muss das globale Minimum in Abhängigkeit von dem Gain $g$ gefunden werden \autocite{noise-shaping}:
\begin{equation}
\setequationentry{Gainoptimierung der Zielfunktion}
(g, m) \Rightarrow \left( \frac{2}{f_s} \int_{0}^{\nicefrac{f_s}{2}} S(m, f) + g\; \text{d} f \right)^2 \to 0
\label{eq:optimize}
\end{equation}
